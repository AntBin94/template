...
from app.models import Producto, ProductoCreate, ProductoPublic, ProductosPublic, ProductoUpdate, Message
...
def read_productos(
    ...
    statement = select(Producto).order_by(col(Producto.created_at).desc()).offset(skip).limit(limit)
